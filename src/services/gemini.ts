...
const schema = {
    ...
    ethicalChecks: {
      ...
      items: {
        ...
        properties: {
          ...
          analysis: { type: Type.STRING, description: "Detailed ethical analysis." }
        },
        required: ["framework", "alignment", "summary", "analysis"]
      }
    },
    roadmap: {
      type: Type.ARRAY,
  ...
  required: ["consensus", "finalScore", "overallAlignment", "swarm", "biases", "xaiTree", "simulation", "ethicalChecks", "roadmap"]
};
...
export async function analyzeDecision(
  ...
  const systemInstruction = `
    ...
    3. XAI Tree: Provide a multi-level logic tree (at least 2 levels deep) explaining the decision path. Mark the 'isHighlight' boolean as true for the specific path of nodes that contribute most to the 'finalScore' (the primary reasoning path).
    4. Simulation: Provide a numerical prediction sequence (e.g., 5-6 points) and a text prediction of outcomes over time.
    5. Ethical Alignment: Check the decision against 2 different ethical frameworks relevant to the domain. Provide a clear alignment status (High, Medium, Low) and a one-sentence summary for each.
    6. Strategic Roadmap: Provide 3 clear sequential phases for implementation.
